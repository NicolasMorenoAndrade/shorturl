...
(defnc app []
  (let [[state set-state] (hooks/use-state {:slug nil :url "" :custom-slug "" :loading? false})
    ...
    (d/div {:class-name "bg-orange-100 grid place-items-center h-screen p-4"}
           (d/div {:class-name "bg-white rounded-lg shadow-md p-8 w-full max-w-md"}
                  (d/h1 {:class-name "text-2xl font-bold text-orange-800 mb-6 text-center"}
                        "URL Shortener")
                  ...
                  (if (:slug state)
                    (d/div {:class-name "text-center"}
                           ...
                           (d/a {:href redirect-link
                                 :class-name "text-orange-500 hover:text-orange-600 font-medium text-lg break-all"}
                                redirect-link)
                           (d/button {:class-name "mt-6 w-full bg-orange-600 hover:bg-orange-700 text-white py-2 px-4 rounded transition-colors duration-200"
                    ...
                    (d/form {:on-submit (fn [e] (.preventDefault e) (handle-shorten-url))
                            ...
                            (d/div
                             ...
                             (d/input {:id "url-input"
                                       ...
                                       :class-name "w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-transparent"
                            ...
                            (d/button {:type "submit"
                                       :disabled (:loading? state)
                                       :class-name (str "w-full bg-orange-600 "
                                                        (if (:loading? state) "opacity-70 cursor-not-allowed" "hover:bg-orange-700 cursor-pointer") " text-white py-2 px-4 rounded-md transition-colors duration-200 font-medium mt-2")}
                                      (if (:loading? state)
